...
INSERT INTO public.contributors (user_id, user_name, image, link, identity, description, include, created_at, updated_at)
SELECT 'beansamuel', '趙啟翔', 'https://github.com/BeanSamuel.png', 'https://github.com/BeanSamuel', NULL, NULL, false, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM public.contributors WHERE user_id = 'beansamuel');

INSERT INTO public.contributors (user_id, user_name, image, link, identity, description, include, created_at, updated_at)
SELECT 'yueyue14', '鍾宛玲', 'https://github.com/YUEYUE14.png', 'https://github.com/YUEYUE14', NULL, NULL, false, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM public.contributors WHERE user_id = 'yueyue14');

INSERT INTO public.contributors (user_id, user_name, image, link, identity, description, include, created_at, updated_at)
SELECT 'kevensdreamland', '林楷鈞', 'https://github.com/KevensDreamLand.png', 'https://github.com/KevensDreamLand', NULL, NULL, false, NOW(), NOW()
...
INSERT INTO public.query_charts (
  ...
  'static', NULL, 0, NULL,
  '臺北市政府衛生局 / 新北市政府衛生局 / 教育部 / 臺北市政府產業發展局 / 衛生福利部食品藥物管理署',
  '雙北 RFSRAI 區域食品安全風險評估指標',
  '整合四項食安指標，以加權公式計算雙北各行政區的複合風險分數（RFSRAI = 0.45H + 0.3E + 0.25S − 0.05M）。H 風險基礎值（權重 0.45）：各區不合格稽查筆數占雙北全區比例，比例越高代表歷史違規密度越高。E 學生暴露量（權重 0.3）：各區學校數占雙北全區比例，學校越密集出事時受影響師生越多。S 物流風險（權重 0.25）：各區食品工廠數占全區比例，供應商集中度越高一旦出問題波及範圍越廣。M 管理扣分（權重 −0.05）：通過二級品管驗證（GHP、HACCP）之業者比例，自主管理能力越好整體風險越低。分數乘以 100 以整數點數（pts）呈現。',
  '識別雙北高風險行政區，協助衛生局優先配置稽查資源。大安區（78 pts）、中山區（72 pts）、板橋區（70 pts）位居前三，主因是學校密集且不合格稽查比例偏高；泰山區（5 pts）、八里區（5 pts）風險相對最低。指數越高代表優先稽查必要性越高，可搭配組件 302（不合格統計）交叉驗證後配置資源。',
  '{https://imap.health.gov.taipei/App_Prog/Analysis1.aspx,https://fsmc.ntpc.gov.tw/DigitalMap/PublicWebsite,https://fatraceschool.k12ea.gov.tw/frontend/search.html,https://data.gov.tw/dataset/121252,https://data.gov.tw/dataset/127936,https://data.gov.tw/dataset/48579}',
  '{og-matcha,beansamuel,yueyue14,kevensdreamland,baker-ku,codefest2026,doit,ntpc}',
  NOW(), NOW(),
  'two_d',
...
INSERT INTO public.query_charts (
  ...
  'rfsrai_index', NULL, NULL, '{}',
  'static', NULL, 0, NULL,
  '臺北市政府衛生局 / 教育部 / 臺北市政府產業發展局 / 衛生福利部食品藥物管理署',
  '臺北市 RFSRAI 區域食品安全風險評估指標',
  '整合四項食安指標，以加權公式計算臺北市各行政區的複合風險分數（RFSRAI = 0.45H + 0.3E + 0.25S − 0.05M）。H 風險基礎值（權重 0.45）：各區不合格稽查筆數占全區比例，比例越高代表歷史違規密度越高。E 學生暴露量（權重 0.3）：各區學校數占全市比例，學校越密集出事時受影響師生越多。S 物流風險（權重 0.25）：各區食品工廠數占全市比例，供應商集中度越高一旦出問題波及範圍越廣。M 管理扣分（權重 −0.05）：通過二級品管驗證（GHP、HACCP）之業者比例，自主管理能力越好整體風險越低。分數乘以 100 以整數點數（pts）呈現。',
  '識別臺北市高風險行政區，協助衛生局稽查資源配置。大安區（78 pts）、中山區（72 pts）、士林區（63 pts）位居前三；南港區（13 pts）風險相對最低。可搭配組件 302（不合格統計）交叉驗證，優先稽查高分行政區。',
  '{https://imap.health.gov.taipei/App_Prog/Analysis1.aspx,https://fatraceschool.k12ea.gov.tw/frontend/search.html,https://data.gov.tw/dataset/121252,https://data.gov.tw/dataset/48579}',
  '{og-matcha,beansamuel,yueyue14,kevensdreamland,baker-ku,codefest2026,doit,ntpc}',
  NOW(), NOW(),
  'two_d',
  $SQL$SELECT district AS x_axis, score AS data
